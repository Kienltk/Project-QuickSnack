...
<script>
    ...
    $('#search_bar').on('input', function() { // Sử dụng sự kiện 'input' thay vì 'submit'
        var searchTerm = $(this).val(); // Lấy giá trị từ ô tìm kiếm
        if (searchTerm != '') { // Chỉ gửi yêu cầu nếu ô tìm kiếm không trống
            $.ajax({
                type: 'POST',
                url: '../../views/includes/search.php', // Đường dẫn tới script xử lý tìm kiếm
                data: {
                    action: 'search_term',
                    data: searchTerm
                },
                success: function(response) {
                    $('#search_results').html(response); // Hiển thị kết quả tìm kiếm
                }
            });
        } else {
            $('#search_results').html(''); // Nếu ô tìm kiếm trống, xóa kết quả hiển thị
        }
    });
});
